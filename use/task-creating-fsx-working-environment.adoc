...
* IAM 角色的 ARN ，为 Cloud Manager 提供为 ONTAP 工作环境创建 FSX 所需的权限。请参见 link:task-setting-up-permissions-fsx.html["将 AWS 凭据添加到 Cloud Manager"] 了解详细信息。
...
.. 如果您的帐户中没有 IAM 角色，请单击 * 凭据页面 * ，然后按照向导中的步骤使用 FSX for ONTAP 凭据为 AWS IAM 角色添加 ARN 。请参见 link:task-setting-up-permissions-fsx.html["将 AWS 凭据添加到 Cloud Manager"] 了解详细信息。
+
